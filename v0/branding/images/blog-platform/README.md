...
takes a slice off the top and bottom, and the square thumbnail beside a row
takes a harder one off both sides, so the title stays in a central safe area.
The masthead and the keyword footer are full-width-only; both are cropped away
by the square.
...
The masthead sits 48 pixels below the top edge and the keyword footer 49 above
the bottom, so the two read as a matched pair framing the card. That is the
reason the domain is centred up there rather than tucked in a corner, which was
the other candidate and which fought the centred composition.

Each entry below is the live `-v2` file. The matching `-v1` file sits beside it
...
One thing the artwork says that the registry does not: the publication card
reads "Blog Platform", while the publication is titled "Blog Platform Docs". The
wording predates that rename and was carried through the v2 redraw unchanged,
because changing what a card says is an editorial decision rather than a design
