...
The accordion in post content is the deliberate exception to hand-built state: it is a native details element, so the browser itself provides the toggle, the keyboard behaviour, and the expanded or collapsed announcement, with no script and nothing for this contract to maintain. The disclosure arrow is a styled decoration marked aria-hidden, rotated on the browser's own open state, and its rotation is switched off for readers who prefer reduced motion. One honest limit: headings inside a closed accordion are invisible to a screen reader's headings list until it is opened, which is part of why headings inside accordions are kept out of the table of contents.
